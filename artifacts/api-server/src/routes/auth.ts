...
/* ============================================================
   ...
   Creates the first company + first admin.

   This endpoint works ONLY when there is no admin account.
   ============================================================ */
...
router.post("/auth/setup-admin", async (req, res) => {
  ...
  if (String(companyName).trim().length < 2) {
    res.status(400).json({
  ...
  try {
    ...
    /*
     * Security rule:
     * If an admin already exists, first setup is closed.
     */
    ...
    const [employeeProfile] = await db
      ...
      .values({
        companyId: company.id,
        employeeCode: `ADMIN-${adminUser.id}`,
    ...
    if (err?.code === "23505") {
      res.status(400).json({
        error: "Email or company already exists",
      });
      return;
    }
...
router.post("/auth/register", (_req, res) => {
  res.status(403).json({
    error: "Company registration is disabled. Use first admin setup.",
  });
});
...
router.post("/auth/register-employee", async (req, res) => {
  ...
  try {
    ...
    /*
     * Automatically find the company's only/first company.
     */
  ...
  async (req: AuthRequest, res) => {
    try {
      ...
      if (!newPassword && !newEmail) {
        res.status(400).json({
          error: "Provide newPassword or newEmail",
        });
        return;
      }
