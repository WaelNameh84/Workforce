...
export default function ReportsPage() {
  ...
  const [yearSel,  setYearSel]  = useState(format(today, 'yyyy'));
  const [monthSel, setMonthSel] = useState(format(today, 'MM'));
  const period = `${yearSel}-${monthSel}`;
  ...
  const { data: attendanceData } = useGetAttendance({
    companyId: cid, startDate: `${period}-01`, endDate: `${period}-31`,
  });
  ...
  const tableData = useMemo(() => {
    ...
    return emps.map(emp => {
      ...
      return {
        ...
        contractType: p?.contractType || emp.contractType || 'monthly',
        present,
        absent,
  ...
  const filteredTableData = useMemo(() => {
    ...
    if (contractTypeFilter !== 'all')
      data = data.filter(r => r.contractType === contractTypeFilter);
    if (statusFilter !== 'all')
  ...
  const deptChartData = useMemo(() => {
    ...
    return deptData.departments.map(d => {
      ...
      const ded      = deptPay.reduce((s, p) => s + parseFloat(p.totalDeductions|| '0'), 0);
  ...
  const trendData = useMemo(() => {
    const base = statsData?.totalNet || 50000;
    return Array.from({ length: 6 }, (_, i) => {
      const d = subMonths(today, 5 - i);
      return {
        month: format(d, 'MMM yyyy'),
        total: Math.floor(base * (0.8 + (i * 0.04))),
      };
    });
  }, [statsData]);
  ...
  // ── Exports ────────────────────────────────────────────────────────────────
  const exportHeaders = [
    'الاسم', 'الرقم', 'القسم', 'الوظيفة', 'نوع العقد',
    'أيام الحضور', 'أيام الغياب', 'ساعات العمل', 'ساعات الإضافي',
    'تأخير (د)', 'خروج مبكر (د)',
    'الراتب الأساسي', 'الاستحقاقات', 'الخصومات', 'صافي الراتب', 'الحالة',
  ];
  const exportRows = () =>
    filteredTableData.map(r => [
      r.name, r.code, r.dept, r.position,
      r.contractType === 'monthly' ? 'شهري' : r.contractType === 'daily' ? 'يومي' : 'بالساعة',
      r.present, r.absent,
      parseFloat(r.workedHours), parseFloat(r.overtimeHours),
      parseFloat(r.lateMinutes), parseFloat(r.earlyMinutes),
      parseFloat(r.basicSalary), parseFloat(r.earnings),
      parseFloat(r.deductions),  parseFloat(r.netSalary),
      r.status,
    ]);

  const handleExportCSV   = () => downloadCsv(`تقرير_رواتب_${period}.csv`, exportHeaders, exportRows() as any);
  const handleExportExcel = () => downloadExcel(`تقرير_رواتب_${period}.xls`, exportHeaders, exportRows() as any);
  const handlePrint       = () => window.print();

  // ── CONTRACT TYPE LABEL ────────────────────────────────────────────────────
  const ctLabel = (ct: string) =>
    ct === 'monthly' ? 'شهري' : ct === 'daily' ? 'يومي' : ct === 'hourly' ? 'بالساعة' : ct;
    ...
    <div className="space-y-6 pb-24">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex items-center gap-2 print:hidden flex-wrap">
          <Button variant="outline" onClick={handlePrint}><Printer className="w-4 h-4 ml-2" /> طباعة / PDF</Button>
          <Button variant="outline" onClick={handleExportCSV}><Download className="w-4 h-4 ml-2" /> CSV</Button>
          <Button variant="outline" onClick={handleExportExcel}><FileText className="w-4 h-4 ml-2" /> Excel</Button>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-3 p-4 rounded-xl glass print:hidden">
        ...
        <Select value={yearSel} onValueChange={setYearSel}>
          <SelectTrigger className="w-[110px]"><SelectValue placeholder="السنة" /></SelectTrigger>
          <SelectContent>
        ...
        <Select value={contractTypeFilter} onValueChange={setContractTypeFilter}>
          <SelectTrigger className="w-[130px]"><SelectValue placeholder="نوع العقد" /></SelectTrigger>
          <SelectContent>
      ...
      <Tabs defaultValue="overview" className="w-full">
        <TabsList className="grid w-full grid-cols-3 max-w-md bg-muted/50 p-1 mb-6 print:hidden">
          <TabsTrigger value="overview" className="rounded-lg data-[state=active]:bg-background data-[state=active]:shadow-sm">
            <LayoutGrid className="w-4 h-4 ml-2" /> ملخص عام
          </TabsTrigger>
          <TabsTrigger value="employees" className="rounded-lg data-[state=active]:bg-background data-[state=active]:shadow-sm">
            <TableIcon className="w-4 h-4 ml-2" /> الموظفون
          </TabsTrigger>
          <TabsTrigger value="charts" className="rounded-lg data-[state=active]:bg-background data-[state=active]:shadow-sm">
            <PieChartIcon className="w-4 h-4 ml-2" /> المخططات
          </TabsTrigger>
        </TabsList>
              ...
              <table className="w-full text-sm text-right">
                <thead className="text-xs text-muted-foreground bg-muted/30 border-b border-border">
                  <tr>
                    ...
                    <th className="px-4 py-3 font-semibold text-center">نوع العقد</th>
                    ...
                    <th className="px-4 py-3 font-semibold text-center">مبكر(د)</th>
                ...
                <tbody className="divide-y divide-border/50">
                  {filteredTableData.map((row) => (
                    <tr key={row.id} className="hover:bg-muted/10 transition-colors">
                      ...
                      <td className="px-4 py-3 text-center">
                        <Badge variant="outline" className="text-xs">{ctLabel(row.contractType)}</Badge>
                      </td>
                      ...
                      <td className="px-4 py-3 text-center font-data text-amber-400">{row.earlyMinutes}</td>
                  ...
                  {filteredTableData.length === 0 && (
                    <tr>
                      <td colSpan={13} className="text-center py-10 text-muted-foreground">لا يوجد بيانات لعرضها</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card className="border border-border/50 shadow-sm">
              <CardHeader>
                ...
                <CardDescription>صافي الرواتب لكل قسم</CardDescription>
              </CardHeader>
            ...
            {/* Chart 3: Earnings vs Deductions by dept */}
            <Card className="border border-border/50 shadow-sm">
              <CardHeader>
            ...
            <Card className="border border-border/50 shadow-sm">
              ...
              <CardContent className="h-[300px] flex items-center justify-center" dir="ltr">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={attPieData} cx="50%" cy="50%" innerRadius={80} outerRadius={110} paddingAngle={5} dataKey="value">
                      {attPieData.map((entry, i) => <Cell key={i} fill={entry.color} stroke="transparent" />)}
                    </Pie>
                    <RechartsTooltip contentStyle={{ backgroundColor: 'var(--card)', borderColor: 'var(--border)', borderRadius: '8px' }} />
            ...
            {/* Chart 5: Overtime hours by dept */}
            <Card className="border border-border/50 shadow-sm">
              <CardHeader>
                <CardTitle className="text-lg">توزيع ساعات الإضافي</CardTitle>
                <CardDescription>إجمالي ساعات العمل الإضافي لكل قسم</CardDescription>
              </CardHeader>
              <CardContent className="h-[300px]" dir="ltr">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={deptChartData} margin={{ top: 10, right: 10, left: 0, bottom: 20 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" vertical={false} />
                    <XAxis dataKey="name" stroke="#888" fontSize={12} tickLine={false} axisLine={false} />
                    <YAxis stroke="#888" fontSize={12} tickLine={false} axisLine={false} tickFormatter={v => `${v}h`} />
                    <RechartsTooltip cursor={{ fill: 'var(--muted)' }} contentStyle={{ backgroundColor: 'var(--card)', borderColor: 'var(--border)', borderRadius: '8px' }} />
                    <Bar dataKey="overtime" fill="#f59e0b" radius={[4, 4, 0, 0]} name="ساعات الإضافي" />
                  </BarChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>

            {/* Chart 6: Deductions breakdown pie */}
            <Card className="border border-border/50 shadow-sm">
              <CardHeader>
                <CardTitle className="text-lg">توزيع الخصومات</CardTitle>
                <CardDescription>تفصيل مكونات إجمالي الخصومات للشهر</CardDescription>
              </CardHeader>
              <CardContent className="h-[300px] flex items-center justify-center" dir="ltr">
                {deductionsPieData.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={deductionsPieData} cx="50%" cy="50%" innerRadius={70} outerRadius={105} paddingAngle={4} dataKey="value">
                        {deductionsPieData.map((entry, i) => <Cell key={i} fill={entry.color} stroke="transparent" />)}
                      </Pie>
                      <RechartsTooltip contentStyle={{ backgroundColor: 'var(--card)', borderColor: 'var(--border)', borderRadius: '8px' }} />
                      <Legend verticalAlign="bottom" height={36} />
                    </PieChart>
                  ...
                  <p className="text-muted-foreground text-sm">لا توجد خصومات مسجّلة لهذه الفترة</p>
                )}
              </CardContent>
            </Card>
...
function StatusBadge({ status }: { status: string }) {
  switch (status) {
    ...
    case 'none':     return <Badge variant="outline" className="text-muted-foreground">لا يوجد</Badge>;
