...
export default function Assets() {
  ...
  const save = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast({ variant: 'destructive', title: t('actions'), description: 'Could not save this asset.' });
    }
  };
  ...
  const remove = async () => {
    ...
    try {
      ...
      toast({ variant: 'destructive', title: t('actions'), description: 'Could not delete this asset.' });
    }
  };

  const summaryCards = [
    { label: t('totalAssets'), value: assets.length, primary: true },
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div><h1 className="text-2xl font-bold tracking-tight">{t('assets')}</h1><p className="text-sm text-muted-foreground">Track company equipment and assignments.</p></div>
        <Button className="gap-2" onClick={() => { setEditing(null); setShowForm(true); }}><Plus className="h-4 w-4" /> {t('addAsset')}</Button>
      </div>
      ...
      <Card className="border-none shadow-sm">
        <CardContent className="p-0">
          <Table><TableHeader><TableRow className="bg-muted/50 hover:bg-muted/50"><TableHead>{t('assets')}</TableHead><TableHead>{t('serialNumber')}</TableHead><TableHead>{t('assignedTo')}</TableHead><TableHead>{t('purchaseDate')}</TableHead><TableHead>{t('status')}</TableHead><TableHead className="text-right">{t('actions')}</TableHead></TableRow></TableHeader>
            <TableBody>
              {isLoading ? <TableRow><TableCell colSpan={6} className="text-center py-8 text-muted-foreground">{t('loading')}</TableCell></TableRow> : !assets.length ? <TableRow><TableCell colSpan={6} className="text-center py-8 text-muted-foreground">{t('noAssetsFound')}</TableCell></TableRow> : assets.map((asset) => <TableRow key={asset.id} onClick={() => setSelected(asset)} className="cursor-pointer hover:bg-muted/30">
      ...
      <DetailDialog open={!!selected} onOpenChange={(open) => !open && setSelected(null)} title={selected?.name || t('assets')} items={selected ? [{ label: t('serialNumber'), value: selected.serialNumber }, { label: 'Type', value: selected.type }, { label: t('assignedTo'), value: selected.assignedToName || t('unassigned') }, { label: t('status'), value: selected.status }, { label: t('purchaseDate'), value: selected.purchaseDate }] : []} />
